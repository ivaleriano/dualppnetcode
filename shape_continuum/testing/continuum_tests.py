...
def parse_args():
    ...
    parser.add_argument(
        "--data_path_test",
        type=str,
        default="../data/adni_surv_mci2ad_bl_balanced_sets/sets/0/test/",
        help="path to testing dataset",
    )
    parser.add_argument(
        "--discriminator_net", type=str, default="hrpnet", help="which architecture to use for discriminator"
    )
    ...
    parser.add_argument(
        ...
        default=["crb", "crb"],
        nargs="*",
        help="list specifying which recalibration block to use in each level (options:none,crb,srb,scrb)",
    )
...
def eval_surv(model, testDataLoader, i_test=0, writer=None):
    with torch.no_grad():
        model.eval()
        for batch_idx, (x, y_event, y_time, y_riskset) in tqdm(
            enumerate(testDataLoader, 0), total=len(testDataLoader), smoothing=0.9
        ):
            shapes, y_event, y_riskset = x.cuda(), y_event.cuda(), y_riskset.cuda()
